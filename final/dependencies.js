...
window.Open_Face_Cube = window.classes.Open_Face_Cube =
class Open_Face_Cube extends Shape    // An open face cube inserts five square strips into its arrays.
{ constructor()  
    { super( "positions", "normals", "texture_coords" );
      for (var i = 0; i < 5; i++) {
        var square_transform = Mat4.rotation( i == 0 ? -Math.PI/2 : 0, Vec.of(1, 0, 0) )
...
window.TorusModified = window.classes.TorusModified =
class TorusModified extends Shape                                         // Build a donut shape.  An example of a surface of revolution.
  { constructor( rows, columns )  
      { super( "positions", "normals", "texture_coords" );
        const circle_points = Array( rows ).fill( Vec.of( .75,0,0 ) )
                                           .map( (p,i,a) => Mat4.translation([ -2,0,0 ])
  ...
  fragment_glsl_code()           // ********* FRAGMENT SHADER *********
    { return `
        ...
        { float a = animation_time, u = f_tex_coord.x, v = f_tex_coord.y;   
          ...
          gl_FragColor = vec4(1, 0, 0, 1);                        // function of the UV texture coordintaes of the pixel and of time.
        }`;
    }
}
